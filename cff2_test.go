...
// TestCFF2Variable exercises the read path on a variable CFF2 font: the
// outlines survive a write/read cycle as CFF2, and no gvar is invented for
// them.
func TestCFF2Variable(t *testing.T) {
	font := debug.MakeVarCFF2Font()
	...
	// some glyph yields a non-empty default outline
	nonEmpty := 0
	for gid := 1; gid < font.NumGlyphs(); gid++ {
...
// makeVarCFF2FontNonUnitMatrix is a variant of debug.MakeVarCFF2Font whose font
